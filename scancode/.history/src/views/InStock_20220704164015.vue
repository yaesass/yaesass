  ...
  <el-tabs v-model="activeName" @tab-click="handleClick">
    <el-tab-pane label="待入库订单" class="counttable" name="待入库订单">
      ...
      <el-table
        ...
        style="width: 95%; margin: 40px auto 0 auto; font-size: 40px"
    ...
    <el-tab-pane label="已入库订单" class="counttable" name="已入库订单">
      <el-table
        ...
        style="width: 95%; margin: 20px auto 0 auto; font-size: 40px"
        :cell-style="totalcellcolor"
        height="550"
        :row-style="{ height: '48px' }"
      >
        ...
        <el-table-column align="center" label="入库时间" width="250px">
          <template slot-scope="scope">
            <span v-html="scope.row.WarehousingTime"></span>
          </template>
        </el-table-column>
      </el-table>
    </el-tab-pane>
      ...
      that.$axios
        ...
        .then(function (response) {
          ...
          if (reqdata.dt != "") {
            ...
            that.getlist(that.activeName); //没有正在入库的订单
          }
          if (e == "first") {
            that.$loading().close();
          }
      ...
      this.$axios
        ...
        .then(function (response) {
          ...
          for (let i = 0; i in temp; i++) {
            if (temp[i].WarehousingTime) {
              temp[i].WarehousingTime = temp[i].WarehousingTime.replace(
                "T",
                "<br/>"
              );
            }
            list.push({
              ...
              OrderId: temp[i].Id,
              RealQuantity: temp[i].RealQuantity,
              WarehousingTime: temp[i].WarehousingTime,
            });
          }
