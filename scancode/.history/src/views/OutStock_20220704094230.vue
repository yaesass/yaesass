...
export default {
  ...
  methods: {
    ...
    message(msg) {
      ...
      this.getVoices = window.speechSynthesis.getVoices();
      console.log(" this.getVoices ", window.speechSynthesis.getVoices());
      msgSpeak.text = msg; // 文字内容
      msgSpeak.lang = "zh-CN"; // 使用的语言:中文
      msgSpeak.volume = 1; // 声音的音量，区间范围是0到1，默认是1。
      msgSpeak.rate = 1.25; // 设置播放语速，范围：0.1 - 10之间    正常为1倍播放
      msgSpeak.pitch = 2; // 表示说话的音高，数值，范围从0（最小）到2（最大）。默认值为1。
      msgSpeak.voice = this.getVoices[2];
      console.log("msgSpeak", msgSpeak);
